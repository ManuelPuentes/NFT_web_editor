...
export class GenerateImagesService {
  ...
  async exec({
    ...
    const assets_data = this.loadCollectionAssetsData({
      assets_list,
      collection_name,
    });
  ...
  loadCollectionAssetsData({
    ...
    Object.keys(assets_list).map((trait) => {
      ...
      assets_list[trait].map((element) => {
        ...
        data[trait][element] = JSON.parse(
          fs.readFileSync(asset_path).toString(),
        );
      });
    });
  ...
  loadCollectionAssetsList({
    collection_name,
  }: {
    collection_name: string;
  }): Record<string, string[]> {
    const assets_list_path = `./collections/${collection_name}/assets_list.json`;
    const assets_list = JSON.parse(
      fs.readFileSync(assets_list_path).toString(),
    );
    return assets_list;
  }
