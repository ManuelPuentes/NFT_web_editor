...
export class SetDrawOrderService {
  ...
  async exec({
    ...
    const old_draw_order: Array<string> = this.loadOldDrawOrder({
      collection_name,
    });

    this.validateNewDrawOrder({ new_draw_order: draw_order, old_draw_order });

    this.overwriteDrawOrderFile({
      collection_name,
      new_draw_order: draw_order,
    });
  }

  private ensureDrawOrderFileExist({
    collection_name,
  }: {
    collection_name: string;
  }) {
  ...
  private overwriteDrawOrderFile({
    collection_name,
    new_draw_order,
  }: {
    collection_name: string;
