...
export class GetAssetsDetailsService {
    ...
    async exec({ collection_name }: { collection_name: string }): Promise<Record<string, Record<string, AssetDetails>>> {

        this.ensureAssetDetailsExist({ collection_name })
        return this.getCollectionAssetsDetails({ collection_name });
    }

    private getCollectionAssetsDetails({ collection_name }: { collection_name: string }): Record<string, Record<string, AssetDetails>> {
    ...
    private ensureAssetDetailsExist({ collection_name }: { collection_name: string }) {
