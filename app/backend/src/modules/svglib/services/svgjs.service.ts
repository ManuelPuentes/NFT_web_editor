...
export class SvgJsService {
    ...
    async generateSvg({ assets_data, metadata, draw_order, canvas_size }: GenerateSVG) {

        const canvas = await this.createCanvas(canvas_size);

        fs.writeFileSync(
            `./collections/luchamask/output/test.svg`,
            canvas.svg()
        );
        ...
        draw_order.map(layer => {
            ...
            this.drawLayer({
                layer_name: layer,
                trait_data: collection_assets_data.get({ trait_name: layer, element_name: metadata[layer] }),
    ...
    private async createCanvas({ width, height }: { width: number, height: number }) {
...
class CollectionAssetsData {
    ...
    get({ trait_name, element_name }: { trait_name: string, element_name: string }): any {
